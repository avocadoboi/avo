...
{
public:
	static constexpr auto NUMBER_OF_ITERATIONS = Count{5};
	static constexpr auto TRIANGLE_HEIGHT = 0.2886751346f; // sqrt(1/12)
...
public:
	...
	SnowflakeFractal()
	{
		...
		auto points = std::vector<Avo::Point<>>{
			{0.f, 0.f},
			{1.f, 0.f},
			{0.5f, TRIANGLE_HEIGHT * 3.f},
			{0.f, 0.f}
		};

		for (auto a : Range{NUMBER_OF_ITERATIONS})
		{
			for (Index b = 1; b < points.size(); b += 4)
			{
				auto point_0 = points[b - 1];
				auto point_1 = points[b];
				points.insert(points.begin() + b, {
					Avo::interpolate(point_0, point_1, 1.f / 3.f),
					0.5f * (point_0 + point_1) + (point_1 - point_0).rotate(Avo::Radians{-Avo::PI<float> * 0.5f}) * TRIANGLE_HEIGHT,
