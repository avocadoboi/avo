...
public:
	App()
	{
		...
		setThemeColor(Avo::ThemeColors::background, Avo::Color(0));
	...
	void draw(Avo::DrawingContext *p_context) override
	{
		...
		p_context->setColor(Avo::Color(0.1f, 1.f, 0.5f));
		...
		p_context->setColor(Avo::Color(0.8f, 0.8f, 0.1f));
		p_context->strokeCircle(Avo::Point<float>().setPolar(m_time, 100.f) += getCenter(), std::sin(m_time)*30.f + 30.f, 2.f);
		p_context->fillCircle(Avo::Point<float>().setPolar(m_time + Avo::PI, 100.f) += getCenter(), std::sin(m_time + Avo::PI)*30.f + 30.f);
	}
};
